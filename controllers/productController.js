...
const removeNullFields = (product) => {
...
const addProductsController = async (req, res) => {
    ...
    // Separate products with and without barcodes
    ...
    products.forEach((product, index) => {
        ...
        // Remove null fields
        const cleanedProductData = removeNullFields(productData);

        if (product.barcode) {
            productsWithBarcodes.push(cleanedProductData);
        } else {
            productsWithoutBarcodes.push(cleanedProductData);
        }
    });

    try {
        // Insert products without barcodes
        if (productsWithoutBarcodes.length > 0) {
            await ProductModel.insertMany(productsWithoutBarcodes);
        }

        // Upsert products with barcodes
        for (const product of productsWithBarcodes) {
            await ProductModel.updateOne(
                { barcode: product.barcode }, // Filter by barcode
                { $setOnInsert: product },    // Set only if not already present
        ...
        return res.status(500).json({ success: false, message: 'Error saving products', error });
    }
};
